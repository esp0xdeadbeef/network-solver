# ./lib/fabric/invariants/node-roles.nix
{ lib }:
...
let
  ...
  nodeNamesByRole = role: nodes:
    builtins.attrNames (lib.filterAttrs (_: n: isRole role n) nodes);
...
in
{
  check = { site, ... }:
    let
      ...
      supported = [ "core" "access" "policy" "upstream-selector" ];

      badRoles =
        lib.filter
          (n:
            let r = nodes.${n}.role or null;
            in r == null || !(lib.elem r supported))
          (builtins.attrNames nodes);

      _rolesOk =
        assert_
          (badRoles == [ ])
          "invariants(node-roles): unsupported or missing role on node(s): ${lib.concatStringsSep ", " (map (n: "${n}=${nodes.${n}.role or "null"}") badRoles)}";
        ...
        builtins.seq _rolesOk (
          builtins.seq _exactlyOnePolicy (
            builtins.seq _upstreamOptionalButUnique (
              builtins.seq _atLeastOneCore (
                builtins.seq _accessOnlyNetworks (
                  builtins.seq _accessDisjoint true
                )
              )
            )
          )
