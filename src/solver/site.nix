...
let
  ...
  accessUnits = lib.unique (map (a: a.unit) (site.attachment or [ ]));
  orderedUnits = lib.unique (lib.concatMap (
    p:
    if builtins.isList p && builtins.length p == 2 then p
    else throw "network-solver: transit.ordering must contain 2-element pairs"
  ) ordering);
  allUnits = lib.unique (orderedUnits ++ accessUnits ++ builtins.attrNames (site.routerLoopbacks or { }));

  rolesResult = rolesMod.compute { inherit lib site enterprise siteId ordering accessUnits allUnits; };
  wanResult = wanMod.build {
    inherit lib site localPool rolesResult;
    roleFromInput = rolesResult.roleFromInput;
    nodesBase = site.units or site.nodes or { };
  };
  enforcementResult = enfMod.build { inherit lib site rolesResult wanResult; };
  topoResult = topoMod.build { inherit lib site siteId enterprise ordering p2pPool rolesResult wanResult enforcementResult; };
in
